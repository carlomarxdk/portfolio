...
window.addEventListener("resize", function (event) {
    ...
    radiusLength = canvas.width * canvas.height / 8000;
    createParticles();
...
function createParticles() {
    particlesArray = [];
    let noOfParticles = canvas.width * canvas.height / 4500;
    ...
    for(let i=0;i<particlesArray.length;i++){
        for(let j=i;j<particlesArray.length;j++){
            ...
            if (distance < vicinityDist){
                ...
                ctx.strokeStyle="rgba(64,128,16,"+ opacity +")";
                ctx.lineWidth=1;
                ctx.beginPath();
